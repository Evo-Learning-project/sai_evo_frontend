<template>
	<DraggablePopup
		...
		:inForeground="true"
	>
		<div class="mt-3" :style="'max-height:' + (mediaQueryMdMatches ? '30' : '15') + 'rem'">
			<div v-if="helpCenterSelectedArticleId === null">
				<div v-if="courseId" class="flex items-center px-6 py-4 mb-2 -mx-6 bg-light place-content-evenly">
					<p>{{ $t("help.take_the_tour") }}</p>
					<Btn @click="$emit('startTour')">{{ $t("help.start_tour") }}</Btn>
				</div>

				<!-- relevant articles -->
				<div v-if="relevantArticles.length > 0" class="flex items-center mb-4 space-x-1.5">
				...
				>
					<div class="w-8 h-8 pt-0.5 pl-1.5 mr-3 -mt-0.5 rounded-full bg-primary bg-opacity-20">
						<span class="text-xl text-primary material-icons-outlined icon-light"> article </span>
					</div>
					<p>{{ article.title }}</p>
				</div>

				<!-- other articles-->
				<div v-if="irrelevantArticles.length > 0" class="flex items-center my-4 space-x-1.5">
...
export default defineComponent({
	...
	computed: {
		...
		sortedArticles(): HelpCenterArticle[] {
			return [...getArticles()].sort((a, b) => this.getArticleRelevance(b) - this.getArticleRelevance(a));
		},
