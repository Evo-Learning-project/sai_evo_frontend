<template>
  <div>
    ...
    <ExerciseSolution class="w-full my-4" v-for="solution in shownSolutions"
      :key="'e-' + exercise.id + '-solution-' + solution.id" :solution="solution" :exercise="exercise"
    ...
    <div class="flex flex-col">
      <Btn @click="showAll = true" :size="'xs'" :variant="'primary-borderless'" class="mr-auto -ml-1"
        v-if="!showAll && nonDraftSolutions.length > SHOW_INITIALLY_COUNT">
        {{ $t("exercise_solution.show_all") }} ({{
            nonDraftSolutions.length
        }})</Btn>
      <Btn @click="onAddSolution()" v-if="(exercise.solutions ?? []).length > 0" :variant="'primary'"
        class="mt-4 mr-auto" :size="'sm'" :outline="true">
        <span class="mr-2 text-base material-icons">reviews</span>
        {{ $t("exercise_solution.propose_solution") }}
      </Btn>
    </div>
    ...
    <div v-if="editingSolutionId !== null">
      <ExerciseSolutionEditor :publishing="publishing" :saving="saving" :savingError="savingError"
        :modelValue="editingSolution" @updateSolution="onDraftSolutionChange($event.key, $event.value)"
        @close="onClose()" :editorType="editorType">
