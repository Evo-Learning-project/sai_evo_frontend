<template>
  <div style="z-index: 1000" class="fixed top-0 left-0 flex items-center justify-center w-full h-full">
    <div style="width: 100vw !important; height: 100vh !important"
      class="fixed z-10 w-full h-full transition-none bg-gray-900 opacity-80"></div>
    <div style="min-height: 24rem; max-height: 40rem"
      ...
      <div class="flex items-center w-full mb-8">
        ...
        <Btn :variant="'icon'" class="ml-auto" :outline="true" @click="$emit('close')" :tooltip="$t('misc.close')"><span
            class="material-icons-outlined">close</span></Btn>
      </div>
      <div class="flex w-full pb-10 space-x-8 overflow-y-auto">
        <!-- editor section -->
        <div class="w-full">
          <TextEditor v-if="editorType === 'text'" :tall="true" class="mt-auto" :modelValue="modelValue.content"
            :placeholder="$t('exercise_solution.your_solution_placeholder')"
            @update:modelValue="onUpdate('content', $event)" />
          <CodeEditor v-else :modelValue="modelValue.content" @update:modelValue="onUpdate('content', $event)"
            :language="editorType" />
        </div>
        <!-- exercise section -->
